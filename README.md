...
Бэкенд учебной платформы anyforms. Стек и паттерны — как в anyforms-back:
...
- **Авторизация по коду с почты** (NotiSend, как в anyforms-back):
  - `POST /api/auth/request-code` `{email}` — код на почту (TTL 10 мин, 5 попыток,
    повтор не чаще раза в 30 секунд).
  - **Доступ клиента: сначала своя база, потом anyforms-back.** Знакомый студент
    с `active=true` входит без внешнего запроса; `active=false` (отключён админом) — 403.
    Незнакомый email проверяется в anyforms-back: `GET /api/tech/course-access?email=…`
    ...
    Если anyforms-back не ответил на проверку нового email — 503.
...
- **S3 опционален**: без `S3_ACCESS_KEY_ID` приложение стартует, но `/api/admin/upload` вернёт 503.
  Бакет свой — `edu-anyforms` (не общий с anyforms-back).
...
- Модуль «открыт», если `opens_at IS NULL` или момент открытия наступил — статус считается
  на лету. Время задаётся в админке по МСК (`MskTime`), хранится как `TIMESTAMPTZ`.
...
Такой email проверку покупки в anyforms-back не проходит и получает роль ADMIN.
